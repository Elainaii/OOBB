...
def get_course_students(tid, cid):
    ...
    data, num = services.get_course_students(tid, cid, page_number, page_size, filters)
    return {'code': 0, 'message': 'success','page':page_number,'size':page_size , 'total_num': num ,'data': data}
...
def get_homeworks(tid, cid):
    ...
    data, num = services.get_homeworks(tid, cid, page_number, page_size, filters)
    return {'code': 0, 'message': 'success','page':page_number,'size':page_size , 'total_num': num ,'data': data}
...
# TODO：开一门全新的课程
# 开一门新课程，需要提交课程名，学分
@teacher_bp.route('/teacher/<int:tid>/courses/new', methods=['POST'])
def new_course(tid):
    data = request.json
